...
*/
template <class T>
bool walkingrelax(Edge<T> *edge) { // d[u] + w(u,v) < d[v]
    if (edge->getOrig()->getWalkingDist() + edge->getWalkingWeight() < edge->getDest()->getWalkingDist()) { // we have found a better way to reach v
        edge->getDest()->setWalkingDist(edge->getOrig()->getWalkingDist() + edge->getWalkingWeight()); // d[v] = d[u] + w(u,v)
...
static std::vector<T> getWalkingPath(Graph<T> * g, const int &origin, const int &dest) {
    ...
    if (v == nullptr || v->getWalkingDist() == INF) { // missing or disconnected
        return res;
    }
