...
inline void readBatchModeNormal(const std::string &filename, std::string &mode, int &source, int &destination) {
    ...
    while (std::getline(file, line)) {
        //while we are able to read each line of the file
...
inline void readBatchModeComplex(const std::string &filename, std::string &mode, int &source, int &destination,
    ...
    while (std::getline(file, line)) {
        ...
        if (std::getline(iss, leading, ':') && std::getline(iss, value)) {
            ...
            } else if (leading == "AvoidSegments") {
                //this logic now will be different
                std::istringstream segments(value); //now use the value as input string
                std::string segment;
                while (std::getline(segments, segment, ')')) {
                    //we didn't reach the end of the value string
